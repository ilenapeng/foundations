...
apikey = 'c10f2d77c46219c362adbdb3e0e20504'
...
url = 'http://ws.audioscrobbler.com/2.0/?method=artist.search&artist=lil&api_key=c10f2d77c46219c362adbdb3e0e20504&limit=50&format=json'
...
# print(data['results']['artistmatches'])
# print(data['results']['artistmatches']['artist'])

# Confirming that I did get 50 artists
# print(len(data['results']['artistmatches']['artist']))
...
for artist in data['results']['artistmatches']['artist']:
    sum_listeners = sum_listeners + int(artist['listeners'])
print(f'{sum_listeners:,}')
...
# print(data['results']['artistmatches']['artist'][0]['image'][-2]['#text'])
...
# Lil John's is a95384b1-6aec-468c-ae0d-8c6daf87c4c2
...
url = 'http://ws.audioscrobbler.com/2.0/?method=artist.getInfo&mbid=a95384b1-6aec-468c-ae0d-8c6daf87c4c2&api_key=c10f2d77c46219c362adbdb3e0e20504&format=json'
...
# print(data['artist']['tags']['tag'][0]['name'])
...
for mbid in mbids:
    print(f"http://ws.audioscrobbler.com/2.0/?method=artist.getInfo&mbid={mbid}&api_key=c10f2d77c46219c362adbdb3e0e20504&format=json")
...
for mbid in mbids:
    url = (f"http://ws.audioscrobbler.com/2.0/?method=artist.getInfo&mbid={mbid}&api_key=c10f2d77c46219c362adbdb3e0e20504&format=json")
    urls.append(url)
...
# Couldn't replicate the exact results below, but this is what I came up with
for url in urls[:3]:
    response = requests.get(url)
    data = response.json()
    print(data['artist']['name'])
    artist_tags = data['artist']['tags']['tag']
    for artist_tag in artist_tags:
        if artist_tag['name'] in ['Hip-Hop', 'rap', 'swag', 'crunk' 'Dirty South']:
            print('is a rapper')
            break
        else:
...
print(rapper_count/total_artists)
